...
(function(angular){
    ...
    angular.module('app').constant('datePickerConfig', {
    ...
    angular.module('app').filter('time',function () {
            ...
            template: '' +
                '<div class="date-range">' +
                    '<table>' +
                        '<tr>' +
                            '<td valign="top">' +
                                '<date-picker model="start" date after="start" before="end" min-view="date" max-view="date"></date-picker>' +
                            '</td>' +
                            '<td valign="top">' +
                                '<date-picker model="end" date after="start" before="end" min-view="date" max-view="date"></date-picker>' +
                            '</td>' +
                        '</tr>' +
                    '</table>' +
                '</div>',
    ...
    angular.module('app').directive('datePicker', ['datePickerConfig', function (datePickerConfig) {
        return {
            restrict: 'E',
            replace: true,
            scope: {
            ...
            template: '' +
                '<div class="date-picker" ng-switch="view">' +
                    ...
                    '<div ng-switch-when="minutes">' +
                        '<table>' +
                            '<thead>' +
                                '<tr>' +
                                    '<th ng-click="prev()">‹</th>' +
                                    '<th colspan="5" class="switch" ng-click="setView(\'hours\')">{{ date|date:"dd MMMM yyyy" }}</th>' +
                                    '<th ng-click="next()">›</i></th>' +
                                '</tr>' +
                            '</thead>' +
                            '<tbody>' +
            ...
            link: function (scope, element, attrs) {
                ...
                scope.setDate = function (date) {
                    ...
                    if ((!nextView || partial) || scope.model) {
                        ...
                        // Bingo
                        scope.$emit('app:set-date', scope.model, scope.view);
                    }
    ...
    angular.module('app').constant('dateTimeConfig', {
        template: function (attrs) {
            return '' +
                '<date-picker model="' + attrs.ngModel + '" ' +
    ...
    angular.module('app').directive('dateTimeAppend', function () {
        return {
    ...
    angular.module('app').directive('dateTime', ['$compile', '$document', '$filter', 'dateTimeConfig', '$parse', function ($compile, $document, $filter, dateTimeConfig, $parse) {
        ...
        return {
            ...
            link: function (scope, element, attrs, ngModel) {
                ...
                function clear() {
// console.log('clear:', picker, container);
                    if (picker) {
                        picker.remove();
                        picker = null;
                    }
                ...
                function showPicker() {
                    ...
                    scope.emitId = 'app:date' + (attrs.id ? ':' + attrs.id : '');

                    scope.$on('app:set-date', function ($event, date, view) {
                        updateInput();
                        scope.$emit(scope.emitId, date);
                    ...
                    if (position === 'absolute') {
                        ...
                        // relative
                        container = angular.element('<div class="date-picker-wrapper"></div>');
